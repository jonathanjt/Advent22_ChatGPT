...
fn main() {
    ...
    let mut common_types = HashSet::new();
    let mut priority_sum = 0;

    for line in reader.lines() {
        let contents = line.unwrap();
        let set1: HashSet<String> = contents.chars().take(contents.len() / 2).map(|c| c.to_string()).collect();
        let set2: HashSet<String> = contents.chars().skip(contents.len() / 2).map(|c| c.to_string()).collect();
        let intersection: HashSet<_> = set1.intersection(&set2).cloned().collect();
        common_types.extend(intersection);
    }

    for item_type in &common_types {
        let priority = if item_type.chars().next().unwrap().is_ascii_lowercase() {
            item_type.to_ascii_lowercase().chars().next().unwrap() as u32 - 'a' as u32 + 1
        } else if item_type.chars().next().unwrap().is_ascii_uppercase() {
            item_type.to_ascii_uppercase().chars().next().unwrap() as u32 - 'A' as u32 + 27
        } else {
    ...
    println!("Item types that appear in both compartments: {:?}", common_types);
